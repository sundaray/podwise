...
export async function generateMetadata({
  ...
  const metadata: Metadata = {
    ...
    description:
      "Discover key insights and actionable takeaways from the Ed Mylett podcast episodes.",
    alternates: {
      canonical: `https://podwise.org/podcasts/ed-mylett${
        page > 1 ? `?page=${page}` : ""
      }`,
    },
  };
...
export default async function edMylettPodcastPage({
  searchParams,
}: {
  searchParams: Promise<SearchParams>;
}) {
  const host = "ed-mylett";
  const awaitedSearchParams = await searchParams;
  const {
    page: currentPage,
    tier,
  ...
  const sortedPodcasts = [...edMylettPodcastList].sort((a, b) => {
  ...
  return (
    <div className="group mx-auto max-w-6xl px-4">
      ...
      <p className="mx-auto mb-20 max-w-5xl text-center text-lg/7 text-balance text-gray-700">
        The Ed Mylett Show showcases peak-performers across all industries,
        sharing their journey, knowledge and thought leadership. Hosted by Ed
        Mylett, one of the world&apos;s top business leaders and peak
        performance experts, the show provides motivation, inspiration and
        practical steps to help you become the best version of yourself.
      </p>
      <PodcastSearch
