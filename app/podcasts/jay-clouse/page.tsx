...
export async function generateMetadata({
  ...
  const metadata: Metadata = {
    title: "Creator Science with Jay Clouse",
    description:
      "Discover key insights and actionable takeaways from the Creator Science podcast episodes.",
    alternates: {
      canonical: `https://podwise.org/podcasts/jay-clouse${
        page > 1 ? `?page=${page}` : ""
      }`,
    },
  };
...
export default async function JayClousePodcastPage({
  searchParams,
}: {
  searchParams: Promise<SearchParams>;
}) {
  const host = "jay-clouse";
  const awaitedSearchParams = await searchParams;
  const {
    page: currentPage,
    tier,
  ...
  const sortedPodcasts = [...jayClousePodcastList].sort((a, b) => {
  ...
  return (
    <div className="group mx-auto max-w-6xl px-4">
      ...
      <p className="mx-auto mb-20 max-w-5xl text-center text-lg/7 text-balance text-gray-700">
        Creator Science helps you become a smarter creator. Hosted by Jay
        Clouse, this Signal award-winning podcast explores proven strategies to
        help you grow your audience, generate more revenue, and break through
        the noise. Each week features conversations with today&apos;s top
        professional creators, sharing practical strategies and insights for
        building a successful creative career in an increasingly competitive
        landscape.
      </p>
      <PodcastSearch
