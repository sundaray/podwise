...
export async function generateMetadata({
  ...
  const metadata: Metadata = {
    title: "How I Write by David Perell",
    description:
      "Discover key insights and actionable takeaways from David Perell podcast episodes.",
    alternates: {
      canonical: `https://podwise.org/podcasts/david-perell${
        page > 1 ? `?page=${page}` : ""
      }`,
    },
  };
...
export default async function DavidPerellPodcastPage({
  searchParams,
}: {
  searchParams: Promise<SearchParams>;
}) {
  const host = "david-perell";
  const awaitedSearchParams = await searchParams;
  const {
    page: currentPage,
    tier,
  ...
  const sortedPodcasts = [...davidPerellPodcastList].sort((a, b) => {
  ...
  return (
    <div className="group mx-auto max-w-6xl px-4">
      ...
      <p className="mx-auto mb-20 max-w-5xl text-center text-lg/7 text-balance text-gray-700">
        How I Write is a podcast hosted by David Perell, a writer, teacher, and
        podcaster known as &quot;The Writing Guy.&quot; The show features
        in-depth conversations with top writers about their creative processes,
        exploring the methods and techniques behind great writing. Guests
        include prominent figures like Sam Altman, Tim Ferriss, and Marc
        Andreessen, who share insights into how they develop ideas, structure
        their thoughts, and bring their words to life.
      </p>
      <PodcastSearch
