function generatePodcastList(): void {
  ...
  const podcastsDir = path.join(process.cwd(), "app/podcasts/tim-ferriss");

  // Define the output file path - adjust this to your project structure
  const outputFilePath = path.join(
    process.cwd(),
    "podcast-list/tim-ferriss.ts",
  );
...
export const timFerrissPodcastList = ${JSON.stringify(podcasts, null, 2)};
`;
